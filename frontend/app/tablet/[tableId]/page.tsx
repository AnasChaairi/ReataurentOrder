...
export default function TabletPage() {
  ...
  const { table, isLoading, error, initializeTable, resetForNextCustomer } = useTablet();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-baristas-cream">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-16 w-16 border-4 border-baristas-brown border-t-transparent mb-6"></div>
          <h2 className="text-2xl font-semibold text-baristas-brown-dark mb-2">
            Loading...
          </h2>
  ...
  // Main tablet interface with view states
  return (
    <>
      ...
      {viewState === "confirmation" && orderResult && (
        <TabletConfirmation orderResult={orderResult} onNewOrder={handleNewOrder} tableId={tableId} />
      )}
    </>
  );
}
